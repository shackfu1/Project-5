...
def value_to_ipv4(addr):
    ...
    ipv4addr = ""
    ipv4addr += str((addr >> 24) & 0xff)
    ipv4addr += "."
    ipv4addr += str((addr >> 16) & 0xff)
    ipv4addr += "."
    ipv4addr += str((addr >> 8) & 0xff)
    ipv4addr += "."
    ipv4addr += str((addr >> 0) & 0xff)
    return  ipv4addr
...
def my_tests():
    ...
    print(ips_same_subnet("10.23.121.17", "10.23.121.225", "/24"))
    print(get_network(0x01020304,0xffffff00))
    print(find_router_for_ip({
        "1.2.3.1": {
            "netmask": "/24"
        },
        "1.2.4.1": {
